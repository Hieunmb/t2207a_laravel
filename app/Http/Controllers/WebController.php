...
//        $category=Category::findOrFail($id);
    ...
    public function placeOrder(Request $request)
    {
        $request->validate([
            ...
            "required" => "Vui lòng điển đầy đủ thông tin",
        ...
        $order = Order::create([
            ...
            "total" => $total,
            "payment_method" => "COD",
//        "is_paid"=>$request->get("firstname"),
//        "status"=>$request->get("firstname")
        ]);
        foreach ($products as $item) {
    ...
    public function successTransaction(Order $order,Request $request){
        $order->update(["is_paid"=>true,"status"=>1]);// da thanh toan, trang thai ve xac nhan
        return redirect()->to("/thank-you/".$order->id);
    }
